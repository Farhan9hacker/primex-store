...
import { DollarSign, Clock, ShoppingCart, Users } from "lucide-react"
...
async function getStats() {
    try {
        ...
        try {
            // Since we are server-side, we can call the logic directly or fetch the internal API.
            // Fetching local API requires absolute URL or headers. 
            // Better to duplicate logic lightly or use a helper, but for now let's just use the API if we have base URL,
            // OR simpler: query DB directly here? 
            // Actually, we can just query the DB for the token and check.
        ...
        return { revenue: 0, todayRevenue: 0, pending: 0, failed: 0, users: 0, onlinePlayers: 0, recent: [], discordBot: { success: false, username: '', error: 'Error' } }
    }
}
                ...
                <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className={`relative ${stats.discordBot.success ? 'text-indigo-500' : 'text-red-500'}`}>
                            {/* icon placeholder could go here, relying on text color for now */}
